...
class Scan:
    ...
    def plot_full_slide(self, downsample=8, colors=None):
    ...
    def show_roi_level0(
        ...
        for label, geom in self.polygons.items():
            ...
            for g in geoms:
                ...
                ax.add_patch(MplPolygon(extL, fill=False, linewidth=l_thick, edgecolor=edgecolor))
    
                # holes
                for ring in g.interiors:
                    hole0 = np.asarray(ring.coords, dtype=np.float64)
                    holeL = np.column_stack([(hole0[:,0] - x0_0)/sx, (hole0[:,1] - y0_0)/sy])
                    ax.add_patch(MplPolygon(holeL, fill=False, linewidth=l_thick, edgecolor=edgecolor, linestyle="--"))
